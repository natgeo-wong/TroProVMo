...
for wtgii in wtgvec, sst in sstvec
    ...
    mkpath(projectdir("exp","prm","DGW",expname,runname))
    for imember = 1 : 5
        mstr = @sprintf("%d",imember)
        oprm = projectdir("run","modifysam","prmtemplates","DGW_$(expinit).prm")
        nprm = projectdir("exp","prm","DGW",expname,runname,"member$(mstr).prm")
        open(tprm,"w") do fprm
            open(oprm,"r") do rprm
                s = read(rprm,String)
                s = replace(s,"[xx]" => mstr)
                ...
                s = replace(s,"[wtgstring]" => dampingstrprnt(wtgii))
                s = replace(s,"e+" => "e")
                write(fprm,s)
            end
        end
        ...
        @info "Creating new prm file for DGW $expname $runname ensemble member $imember"
    end
end
